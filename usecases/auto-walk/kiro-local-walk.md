...
5. ✓ **Second kickstart 2026-05-29 21:46 validated round-2 prompt** (exit 0, ~4m25s). All round-2 invariants verified by the produced artifact:
   ...
   - hyp-005's claim cites facts retrievable from the named refs — §11.5 corpus-coverage rule honored after a review-caught fix: the initial YAML omitted the `#AutoDream 兩階段（2026-05-12 內化）` ref that covers the "index 控 200 行內" fact in the claim; ref was supplemented and the missing heading-space typo on a sibling ref was corrected at the same time. All other survivors (006, 007, noteworthy-002) passed cleanly on first emit.
